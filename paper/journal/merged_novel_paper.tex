...
The bulk linearises the same way: its recursion carries a source, and the elimination gives
$\varepsilon A''+A=-\mathfrak g\beta$, the same oscillator with constant forcing. So $B$ and
$C_\lambda$ are available in closed form too, and \textup{(S2)} becomes a computation rather than
an estimate; the observed law is $|C_\lambda|/(xB)\approx(w+1)/2$, which tends to infinity. What
is still missing is uniform control of the discrete-to-continuum error, and the completion of the
\textup{(S2)} computation. We claim neither, and record that \textup{(S1)} holds at leading order
with the closed form verified above.
