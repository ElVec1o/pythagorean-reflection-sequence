...
\begin{proof}
The point group collapses at depth $8$: breadth-first search in $W_4$ with exact rational
arithmetic produces two elements of length $8$ with the same linear part, whence
$u=w_1w_2^{-1}$ of length $16$ with $\pi(\rho_a(u))=I$ and $u\ne1$ in $W_4$, the latter checked
in the Tits representation. Proposition~\ref{prop:transtrick} then places
$c=[u,\,R_0uR_0]$ in $\ker\rho_a$, and $c\ne1$ in $W_4$, again by the Tits representation;
reducing $c$ gives a geodesic of length $54$. The condition $a_1a_3=a_2a_4$ enters only through
the depth-$8$ collision, which it produces for every member of the family.
\end{proof}
...
$a_1a_3\ne a_2a_4$, none of which deviates by depth $12$. The remaining eight failures, at depth
