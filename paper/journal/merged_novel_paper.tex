...
\]
So $M$ is nilpotent for every $t$: both eigenvalues vanish identically. There is no exponential
Liouville--Green scale to extract, and the degeneracy is not confined to isolated turning points
but holds at every $t$, so the leading-order asymptotics carry no information and everything is
decided by subleading terms. This is the same degenerate amplitude problem that the analytic side
of this circle has repeatedly failed to close, and (S1) and (S2) should not be mistaken for a
fresh and easier target: they are that problem in another guise. We record the reduction, and
leave the estimate open.
