...
the classical free-rotation theorem, proved by the $5$-adic argument: for a reduced word $w$ of
...
cosine of a rational multiple of $\pi$ (Niven). A subgroup of a free group is free, and $A$ and
$B^3$ form a basis of a free group of rank two. Hence $\langle\pi\rho_a(A),\pi\rho_a(C)\rangle$ is
...
\begin{proof}
(i) A single faithful tuple gives $N_{\mathrm{gen}}=1$, and Theorem~\ref{thm:faithful} supplies
one; generic rigidity then gives the co-null set and the growth series. (ii) The image
...
Nothing here decides $n\ge4$. The argument used that $W_3$ is virtually free, which holds because
$\Gamma_3$ has no triangle; already $\Gamma_4$ contains the triangle $\{0,2,4\}$, so $W_4$ is not
virtually free and the decomposition of Lemma~\ref{lem:W3struct} has no analogue there.
\end{remark}
