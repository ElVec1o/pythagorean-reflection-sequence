...
So the chain reads: $(R\!-\!J)$ follows from $(S1)$ and $(S2)$; $(S2)$ is
$|C_\lambda|/(xB)=w/2$; both sides are measured; and the scale $w$ itself is fixed by the sharp
size of $T_2$. The last of these is a known HEURISTIC of the companion paper, with the obstruction
already identified: the steepest-descent argument would conclude by invoking a standard remainder
estimate for a simple non-degenerate saddle, but the saddle here is flat, and the bounded
variation of the amplitude is available only on a bounded piece of the contour. The constant and
the remainder exponent are certain to eight digits over $m\le1024$, and no theorem of that paper
consumes them.

The chain therefore does not end in an unexamined gap. It ends at a named statement whose proof
obstruction is diagnosed, and closing $(R\!-\!J)$ by this route requires the flat-saddle estimate
rather than further measurement.
\end{remark}
