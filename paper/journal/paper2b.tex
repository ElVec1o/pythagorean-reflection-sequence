...
numerators; the spectral and sign laws at the travel poles, resting on the inequality
\textup{(T$^\ast_{\mathrm{poles}}$)}, which is proved here by a flux identity, an amplitude bound and
an interval certificate; an orthogonal arithmetic route through Christol's theorem;
...
version, except \textup{(T$^\ast_{\mathrm{poles}}$)}, which is a hypothesis there and is proved
here.
\end{abstract}
...
\item If $\lambda_{m-1}(q_m)<q_m$, then
...
(c) By {\cite[Theorem ``the travel determinant'', \textup{thm:fredholm}]{Bonfioli2026bA}}\textup{(iv)},\textup{(v)} at $\lambda=q$,
...
{\cite[Proposition \textup{prop:qtrig}]{Bonfioli2026bA}} with Lemma~\ref{lem:infpoles}. Their
...
\end{itemize}
So Proposition~\ref{prop:simplepoles} and Proposition~\ref{prop:signlaws}\textup{(a)},\textup{(b)}
are proved, at the standing of Theorem~\ref{thm:Tstar}. Proposition~\ref{prop:signlaws}\textup{(c)}
is proved under its stated extra condition $\lambda_{m-1}(q_m)<q_m$, which would follow from the
shift inequality $\lambda_n\le q^2\lambda_{n+1}$; that inequality is \emph{not} proved here.
