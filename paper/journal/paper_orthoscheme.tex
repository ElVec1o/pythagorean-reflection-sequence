...
Theorem~\ref{thm:len6} requires $n\ge3$. The question they do not answer is whether $\rho_a$ is
...
\]
and $\widehat R_i\widehat R_j$ is a rotation through $2\theta_{i,j}$ about $H_i\cap H_j$.
...
Lemma~\ref{lem:no-length-four}, $P$ contains every vertex $V_m$ with
...
$\langle m_q,m_r\rangle^{\perp}\cap E$ through twice the angle between $m_q$ and $m_r$.
Equality of the axes gives $m_p\perp m_q$ and $m_p\perp m_r$, equality of the angles gives
$m_q\perp m_r$. By Lemma~\ref{lem:normals} the indices $p,q,r$ are then pairwise at distance at
...
must vanish. Each summand is a unit vector and $a_1>0$, so $k\ne1$.
...
$H_i\ne H_j$, so it has order exactly $3$ and $\theta_{i,i+1}=\pi/3$.
\end{proof}

\begin{remark}[Where each hypothesis is used]\label{rem:len6-hyp}
Positivity of the legs enters twice: through Lemma~\ref{lem:independence}, and in step (3)
through $a_1>0$, which is what makes a single occurrence of $R_0$ leave a nonzero translation.
The hypothesis $n\ge3$ enters through the independence of any three normals, and it cannot be
dropped: at $n=2$ the three normals are dependent, and indeed the plane carries kernel elements
of full support, though of length $20$ rather than $6$ (Corollary~\ref{cor:plane}). No
rationality is assumed anywhere in Theorem~\ref{thm:len6}.
\end{remark}
...
tuple. The value $K(a)=6$ is decided by combining the two preceding sections.
...
$a_i\ne a_{i+2}$, the proof of Theorem~\ref{thm:rank2} places $(a_i,a_{i+2})$ on one of the
three quartics of Lemma~\ref{lem:cremona-empty}, which have no such point. If it is an interior
pair with $a_i=a_{i+2}$, Lemma~\ref{lem:diagonal} gives $a_i=a_{i+1}=a_{i+2}$, that is
$t(a)\ge1$. The last statement follows since $K(a)$ is even and at least $6$.
\end{proof}
...
\begin{cases}
3      & \text{if }t\ge1,\\
4      & \text{if }e\ge1\ \text{ and }t=0,\\
\infty & \text{if }e=t=0\ \text{ and }\rho_a\text{ is injective}.
\end{cases}
\]
The first two cases are unconditional. In the third case, if $\rho_a$ is not injective then
$\cd_n(a)\ge4$.
\end{theorem}
...
If $e\ge1$ and $t=0$ then Theorem~\ref{thm:rank2}(ii) exhibits an element of $\ker\rho_a$ of
...
Corollary~\ref{cor:alternative}; and if $\ker\rho_a\ne\{1\}$ then $K(a)\ge8$ by
Corollary~\ref{cor:K-eq-six}, so $\cd_n(a)\ge4$.
\end{proof}
...
Theorem~\ref{thm:len6} (length six is dihedral)  & proved                  & all $n\ge3$, all positive legs \\
...
argument of step (3) are paper proofs. Lemma~\ref{lem:independence},
Lemma~\ref{lem:no-length-four}, Lemma~\ref{lem:diagonal}, Theorem~\ref{thm:rigidity} and
Theorem~\ref{thm:barrier} are paper proofs.
