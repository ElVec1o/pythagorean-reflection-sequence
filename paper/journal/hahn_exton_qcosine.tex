...
\begin{proof}[Sketch]
Newton's first correction gives $1-u_k=F_k(q,1)/\partial_uF_k(q,1)\,(1+O(q^{T_{2k-1}}))$. The
numerator limit is Proposition~\ref{prop:latticevalues}: $(-1)^kF_k(q,1)q^{-T_{2k-1}}\to
1/(q;Q)_\infty$ (cosine), resp.\ $1/(q^3;Q)_\infty$ (sine). In the denominator the terms
$k'=k+j$ and $k'=k-j-1$ carry the same exponent $q^{j(j+1)}$ with opposite signs and weights
$k+j$, $k-j-1$, so
\[
   (-1)^k\partial_uF_k(q,1)\ \longrightarrow\
   \frac{\sum_{j\ge0}(-1)^j(2j+1)\,q^{j(j+1)}}{(q;q)_\infty}
   =\frac{(q^2;q^2)_\infty^{3}}{(q;q)_\infty}
\]
by Jacobi's identity (sine side: $(q^2;q)_\infty$ in place of $(q;q)_\infty$). The quotients
collapse to $1/(Q;Q)_\infty^2$ in both cases: the $(1-q)$'s cancel between
$1/(q^3;Q)_\infty=(1-q)(Q;Q)_\infty/(q;q)_\infty$ and $(q^2;q)_\infty=(q;q)_\infty/(1-q)$.
\end{proof}
