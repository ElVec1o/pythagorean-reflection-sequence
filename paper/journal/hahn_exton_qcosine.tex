...
Hardouin--Singer and Dreyfus--Hardouin--Roques these give a second route to the differential
...
analogue of Siegel's exclusion of $2\lambda$ odd. Second, the classical model is Siegel
\cite[\S4]{Siegel1929}: for rational $\lambda$, every nonzero zero of $J_\lambda$ is transcendental
(see also Lorch--Muldoon \cite{LorchMuldoon}). We have found no result, and no conjecture, in the
...
\noindent The constant $C_1=1/(q;q)_\infty$ is not new: it is the specialisation to the $q$-cosine
($\nu=-\tfrac12$, $p=q^2$) of Morita's connection formula for $J^{(3)}_\nu$ \cite[Thm.~1]{Morita2011},
which solved exactly this connection problem. What is added here is the regularised zero product,
a corollary of it. Step~(3) of the proof below re-derives the zero localisation
...
$v_k=1-q^{\,k(2k+1)}(1+O(q))$ for every $k\ge1$. (Verified: integrality and onsets to order
$200$ for $k\le4$; the closed form to $44$ digits at $q=0.3$.)
\end{proposition}
...
\[
   R\ \le\ \liminf_{n}\mathcal D_n^{1/n}.
\]
