...
Galois-theoretic computation, valid also at algebraic $q$. We complement this with the regularized
product of the positive zeros, $\prod_{k\ge1}z_k(q)\,q^{2(k-1)}=(q;q^2)_\infty$, a corollary of
Morita's connection formula \cite{Morita2011}. We then pose, and give effective evidence for, the arithmetic question that these results
...
care, because the present object sits in the excluded case. Siegel \cite[\S4]{Siegel1929} proves
...
in $q$ and in $N,k_0$. (Identity~\eqref{eq:supermult} was also checked numerically to $40$ digits at
$q=\tfrac13$ and $q=0.7$ for $j\le12$; the earlier version of this proof rested on that check alone,
at a single $j$ and a single $q$, and exhibited no~$c$. The exponent identity and the bound
$i(j-i)\ge j-1$ are machine-checked in \texttt{lean/with\_mathlib/QCosineExponents.lean} as
\texttt{supermult\_exponent} and \texttt{gap\_ge}, together with the Newton-polygon exponents and
their strict convexity, the ledger sum, the triangular law, and the Diophantine separation
underlying the effective rational-exclusion argument; thirteen theorems, no \texttt{sorry}.) These coefficients grow like $q^{-j^2/2}$, whereas a rational
...
The criterion used is the rank-two one of Arreche, Dreyfus and Roques \cite{ADR}, in the
$q$-difference form recorded by Dreyfus, Hardouin and Roques \cite[Prop.~2.9]{DHR2018}.
...
$2.3390$, $2.4045$, $2.4550$ at $q=0.9,0.95,0.99$ against $j_{-1/2,1}^2=(\pi/2)^2=2.4674$, whereas
$z_1(0.99)=2.455\cdot10^{-4}$ against the discarded $(q\pi/2)^2=2.418$.
...
rational functions of $q$, and the relevant entire function lies in the class treated by Hayman.
...
proof; they note in their Remark~2.2 that it is a limit case of Heine's transformation. It is restated
...
(c) Combine (a) and (b): $v_k$ is the Hensel root attached to the edge, and by (b) the first
correction to $v_k=1$ enters at order $\operatorname{ord}_qH(q,q^{1-2k})+1-\bigl((k+1)^2-1\bigr)$,
which is $k(2k+1)$, exactly as on the cosine side with $T_{2k-1}$ replaced by $T_{2k}$.
(Independently verified: integrality and onsets to order $200$ for $k\le4$, and the closed form
of (b) to $44$ digits at $q=0.3$.)
\end{proof}
...
on at every triangular number exactly once, alternating between the families. (Verified through
$u_8$, i.e.\ $T_{15}=120$.)
\end{remark}
...
\]
whose coefficients $1,2,5,10,20,36,\dots$ count two-colored partitions. Measured agreement depth:
order $2k-2$ on the cosine side ($k\le6$) and $2k-1$ on the sine side ($k\le5$).
\end{proposition}
...
\]
the $r$-th term of the lattice sum contributing at order $q^{(k+r)^2-k(2k-1)}$ so that $r\ge1$ is
$O(q^{2k+1})$, and $(Q;Q)_k=(Q;Q)_\infty\bigl(1+O(q^{2k+2})\bigr)$. Agreement depth $2k+1$.
...
\]
by Jacobi's identity $\sum_{j\ge0}(-1)^j(2j+1)q^{j(j+1)}=(q^2;q^2)_\infty^3$. Agreement depth $2k-1$.
...
(Numerically the cosine agreement depth is $2k-2$ through $k=6$ and the sine $2k-1$ through $k=5$,
consistent with the depth-$(2k-1)$ bound above.)
\end{proof}
...
extrapolating to $0.78621$; values computed from $n\le120$ are preasymptotic). Neither
...
an integer power series that is simultaneously non-algebraic, non-holonomic, and of radius $<1$: precisely the class for which no arithmetic technology exists: the Chudnovsky--Andr\'e--Bost theory
...
P\'olya--Szeg\H{o}, Part~VIII), and the general case is the same argument after rescaling. Calegari,
Dimitrov and Tang \cite[Lem.~1.1.2]{CDT} name the two halves the \emph{Liouville lower bound} and
the \emph{Cauchy upper bound} and trace the lineage Borel (1894), P\'olya--Carlson, Dwork,
Bertrandias, Andr\'e, Bost; the hypothesis that the denominators grow at most geometrically is
Christol's \emph{globally bounded} condition, and the $G$-function form of the corollary is in
Rivoal \cite[\S3]{Rivoal2019}. Note also that the inequality is generally \emph{strict}
...
$X^m-e_1X^{m-1}+\cdots$. Their radii increase monotonically, $0.5546,\,0.7356,\,0.7953,\,0.8425$
for $m=1,2,3,4$, improving the ledger factor from $1.803$ to $1.187$; and by
Proposition~\ref{prop:denrad} they are capped at $1$, which the measured values approach
geometrically ($1-R_m$ decaying by a factor $\approx0.77$ per step). The $m\to\infty$ limit is the
...
rational representation of Euler's constant needs denominator $>10^{10000}$, and it rests on
Khinchin's Theorem~17 (every convergent is a best approximation of the second kind); Lagarias
\cite[Thm.~3.15.1]{Lagarias2013} gives the survey-grade statement with proof. What is new here is
only the bound for this particular constant.
\end{proposition}
...
\smallskip\\
\noindent\textbf{Data availability.} All code and numerical certification data supporting the
computational statements are openly available in the repository cited on the title page and
archived on Zenodo (concept DOI \texttt{10.5281/zenodo.20370090}).
\smallskip\\
...
$q$-hypergeometric series II}, in: Analytic Number Theory, Dev.\ Math.\ \textbf{6}, Kluwer, 2002,
17--25.
...
\bibitem{Kedlaya} K.~S.~Kedlaya, \emph{$p$-adic Differential Equations} / \emph{Weil cohomology in
practice}, lecture notes, Lemma~11.1.2.
...
(1995), 551--560.
\bibitem{Rivoal2019} T.~Rivoal, \emph{Les $E$-fonctions et $G$-fonctions de Siegel}, Journ\'ees
math\'ematiques X-UPS 2019, 197--298.
\bibitem{Lindemann1882} F.~Lindemann, \emph{\"Uber die Zahl $\pi$},
Math.\ Ann.\ \textbf{20} (1882), 213--225.

\bibitem{Siegel1929} C.~L.~Siegel, \emph{\"Uber einige Anwendungen diophantischer Approximationen},
Abh.\ Preuss.\ Akad.\ Wiss.\ (1929), Nr.~1; Ges.\ Abh.\ I, 209--266.
