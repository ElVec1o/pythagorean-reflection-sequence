...
$300$; adjacent leading defects cancel in pairs). No Mahler structure underlies it: $u_1$ satisfies
no polynomial relation $P\bigl(q,u_1(q),u_1(q^d)\bigr)=0$ for $d=2,3$ with degrees up to
$(24,2,2)$, $(16,3,3)$, $(10,4,4)$; the system $\{u_1,u_2,u_3\}$ admits no relation of the
analogous bilinear profile (exact linear algebra on the $300$ integer coefficients, all matrices of
full column rank); and no zero of the base-$q^2$ cosine or sine is a rational multiple of
$q^{e}z_i(q)^{a}w_j(q)^{b}$ (or the sine-side analogue) for $|e|\le10$, $a+b\le2$ (numerical
search at $60$ digits, cross-checked at two bases). Mahler's method --- intrinsically $q$-analytic,
with no classical shadow --- thus finds no foothold on the zero curve at these degrees.
