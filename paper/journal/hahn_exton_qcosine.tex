...
$i(j-i)\ge j-1$ are machine-checked as \texttt{supermult\_exponent} and \texttt{gap\_ge};
see \S\ref{sec:lean}.) These coefficients grow like $q^{-j^2/2}$, whereas a rational
...
Cor~\ref{cor:hexagonal} & complete to $k=6$ & \texttt{hexagonal\_onset},
  \texttt{hexagonal\_next\_correction}, \texttt{cosine\_onset}, \texttt{cosine\_onsets}\\
Prop~\ref{prop:sinelattice} & complete to $k=5$ & \texttt{sine\_shift}, \texttt{sine\_onset},
  \texttt{onsets\_differ}, \texttt{onsets\_consecutive}, \texttt{sine\_onsets},
  \texttt{sine\_residual\_vanishes}\\
Prop~\ref{prop:stablelaw} & depths, to $k=6$ & \texttt{stablelaw\_numerator\_depth},
...
computations on large integer matrices and are reported as computations.
\end{itemize}
