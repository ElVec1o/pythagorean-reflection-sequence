...
machine-checked in Lean~4 in \texttt{lean/with\_mathlib/EnvelopeDichotomy.lean} (twelve theorems,
no \texttt{sorry}), and Lemma~\ref{lem:normals} in
\texttt{lean/with\_mathlib/OrthoschemeNormals.lean} (seven theorems, no \texttt{sorry}): the
support pattern \texttt{supp\_disjoint}, the vanishing of the inner product on disjoint supports
\texttt{sum\_eq\_zero\_of\_disjoint}, and the three adjacent inner products, which are nonzero for
every positive leg tuple. That file is what replaces the earlier ``verified symbolically for
$n\le5$''.
