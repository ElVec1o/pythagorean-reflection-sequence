...
\texttt{ModPStateInfinite.lean} certifies Proposition~\ref{prop:modp-state-infinite}. The
pairs $((XY)^n, t_n)$ lie in $Q \times (M/pM)$ and are distinct as soon as their first
components are, so it suffices that $XY$ has infinite order; neither the translation parts
nor the prime $p$ enter, which is why the conclusion is uniform in $p$. For the $3$-$4$-$5$
triangle $XY$ is multiplication by $(3+4i)/5$, and writing $(3+4i)^n = a_n + b_n i$ the
rotation by $n$ steps is trivial exactly when $b_n = 0$.

The standard argument factors $3+4i = (2+i)^2$ and $5 = (2+i)(2-i)$ in $\mathbb{Z}[i]$ and
invokes unique factorization. That machinery is not needed. The recursion
$(a_{n+1}, b_{n+1}) = (3a_n - 4b_n,\, 4a_n + 3b_n)$ fixes $(3,4)$ modulo $5$, since
$3 \cdot 3 + 4 = 13 \equiv 3$ and $4 \cdot 3 + 3 \cdot 4 = 24 \equiv 4$; as $(a_1,b_1) = (3,4)$,
every $b_n$ with $n \ge 1$ is $\equiv 4 \pmod 5$ and in particular nonzero. The formalization
records this one-line induction, the multiplicativity of the norm, the group law, and the
resulting pairwise distinctness of the rotations.

For Proposition~\ref{prop:no-dfinite} the search grid is reduced rather than enumerated.
\texttt{DFiniteReduction.lean} proves that checking all $52$ pairs is unnecessary: if
$(k,m) \le (k',m')$ componentwise then a nonzero $(k,m)$-recurrence yields a nonzero
$(k',m')$-recurrence by padding the coefficient family with zeros. Two things make the padding
work, and they pull in opposite directions: the ansatz grows, while the constraints
\emph{shrink}, since the $(k',m')$ equations run over $k' \le n \le 42$ and that range sits
inside the range $k \le n \le 42$ of the $(k,m)$ equations. Non-existence therefore propagates
downward, and it suffices to treat the maximal pairs. The grid has exactly six,
$(3,7)$, $(4,6)$, $(5,5)$, $(6,4)$, $(7,3)$, $(9,2)$, and the file verifies by kernel
evaluation that every one of the $52$ is dominated by one of them.

The six base cases are supplied as rank certificates by
\texttt{code/zeta\_probe/tools/nodfinite}. A D-finite recurrence is a nonzero solution of a
\emph{homogeneous} system, so non-existence means full column rank; for each pair the tool
exhibits square submatrix rows together with an inverse modulo $2^{31}-1$, and since reduction
modulo a prime can only lower rank, full column rank there gives it over $\mathbb{Q}$. All
$52$ pairs were certified this way before the reduction was proved, so the six are not a
weakening of the search. Importing them into Lean requires the passage from a modular left
inverse to $\det M \neq 0$ over $\mathbb{Z}$, which is not yet formalized; the proposition is
accordingly reduced but not closed, and the reduction itself is unconditional.
...
What is not formalized is stated plainly in each file. The six remaining base cases of
Proposition~\ref{prop:no-dfinite} are certified outside Lean, as described above, and neither
is the passage
...
Proposition~\ref{prop:height-gap} are numerics and are not formalized. The terms
$u_{39}, \ldots, u_{42}$, used by the propositions stated over $u_0, \ldots, u_{42}$, are
recorded with their provenance in \texttt{code/data/u\_terms\_43.txt}; they were recovered
from the stored logarithms, a recovery that reproduces all $38$ overlapping published terms
exactly, and they have not been regenerated by the growth BFS. For
Proposition~\ref{prop:modp-state-infinite} the identification of $XY$ with the rotation by
$(3+4i)/5$ is the setup of the $3$-$4$-$5$ reflection group and is not re-derived in Lean;
what is proved there is that the rotation so described has infinite order.

The two conditional routes, Propositions~\ref{prop:beta2-christol}
and~\ref{prop:beta2-dfinite}, are not formalized and are not candidates for it here: each is
an implication whose antecedent is an open hypothesis and whose engine is Christol's theorem,
which is not available in Mathlib. Stating them in Lean with that theorem assumed would record
no more than the prose already does.
