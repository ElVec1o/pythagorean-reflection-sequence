...
\begin{corollary}
Until the first Euclidean lattice collision, $u_d = F_{d+3}$.
\end{corollary}
...
\texttt{lean/with\_mathlib/SymbolicVerification.lean} discharge each of
the six resulting polynomial identities over
$\mathbb{Q}[a,b]$ by the \texttt{ring} tactic, so the identities hold
with kernel-checked certainty for every right triangle, not merely on a
sample of specializations.
\qed
\end{proof}
...
\begin{proof}
By Lemma~\ref{lem:uniform-detQ}, $\det Q_n = -\prod a_i^2 \ne 0$ for
every positive tuple, so the Lorentzian Coxeter system is
non-degenerate.  Tits' theorem (Humphreys, \emph{Reflection Groups and
Coxeter Groups}, Thm.~5.4) then gives faithfulness of $\rho$.

It remains to rule out extra Coxeter relations $(R_i R_j)^m = 1$ beyond
the perpendicular pairs.  Such a relation forces some non-perpendicular
$\theta_{ij}$ to be a rational multiple of $\pi$.  Each non-perpendicular
$\cos^2\theta_{ij}$ is rational in $a_1, \ldots, a_n$; by the extended
Niven theorem (Conway--Jones 1976), $\theta_{ij} \in \mathbb{Q}\pi$
with $\cos^2\theta_{ij} \in \mathbb{Q}$ forces $\cos\theta_{ij} \in
\{0, \pm 1/2, \pm\sqrt 2/2, \pm\sqrt 3/2, \pm 1\}$.

\emph{Boundary dihedrals.}  $\cos\theta_{01} = b/\sqrt{a^2+b^2}$ (and
its mirror at the other end) takes the values $0, \pm 1$ only at
degenerate legs; $\pm\sqrt 2/2$ forces $a = b$ (excluded by distinct
legs); $\pm 1/2$ forces $b = 0$ or $a = 0$; $\pm\sqrt 3/2$ forces
$b = a\sqrt 3 \notin \mathbb{Q}$.

\emph{Bridge from $\det Q_n$ to Mordell descent.}  We make the logical
chain explicit.  Suppose $w \in \ker(\rho) \setminus \{1\}$.  Since $\rho$
is the affine realisation of a Coxeter system, the translation component
of $\rho(w)$ vanishes, forcing the $(n+1) \times (n+1)$ Lorentzian Gram
matrix $Q_n(w)$ associated with the relation $w$ to be singular:
$\det Q_n(w) = 0$.  Combined with the Schur-complement identity
$\det Q_n = -\prod_{i=1}^n a_i^2$ of Lemma~\ref{lem:uniform-detQ} and
the explicit dependence of $Q_n(w)$ on the legs $a_1, \ldots, a_n$
recorded in Lemma~\ref{lem:perp-pairs}, this forces some
non-perpendicular cosine to be a rational multiple of $\pi$, i.e.\ the
legs $(a_{i-1}, a_i, a_{i+1})$ to satisfy one of the three Diophantine
equations $\mathcal{V}_{1/4}, \mathcal{V}_{1/2}, \mathcal{V}_{3/4}$ below
over $\mathbb{Q}_{>0}$.  Reducing to the rank-$1$ quotient produces an
integral point on the Jacobian elliptic curve $E$ (Cremona label
\texttt{24a1} for $\mathcal{V}_{1/4}$, \texttt{72a2} for
$\mathcal{V}_{3/4}$, and the classical Mordell curve for $\mathcal{V}_{1/2}$).
By Mordell--Weil, $E(\mathbb{Q})$ is finitely generated, and computation
(verified in SageMath in \texttt{code/mordell\_24a1.sage} and
\texttt{code/mordell\_72a2.sage}, with output committed in the project
repository) shows all rational points are torsion of the form
$(0, \pm a_i)$ or analogues, all of which correspond to the trivial
relation $w = 1$ (equivalently $a_i c = 0$ or $a_i = c$, both excluded
by the pairwise-distinct hypothesis).  Hence $\ker(\rho) = \{1\}$.  The
chain is: \emph{kernel element $\Rightarrow$ Diophantine equation
$\Rightarrow$ rational point on $E$ $\Rightarrow$ finite generation
$\Rightarrow$ all rational points trivial $\Rightarrow$ $\rho$ faithful.}

\emph{Interior dihedrals.}  In the staircase coordinates, every
interior $\cos^2\theta_{i,i+1}$ depends on at most three consecutive
legs and reduces (after the quadratic-in-flanking-leg-squared analysis)
to one of the three Diophantine varieties
\[
...
\]
in the two flanking legs.  All three are provably empty over
distinct-coordinate $\mathbb{Q}_{>0}^2$:
\begin{itemize}[leftmargin=*,itemsep=0.1em]
\item $\mathcal{V}_{1/2}$ by Mordell's classical rank-$0$ descent on
the quartic $a^4 + 6 a^2 c^2 + c^4 = k^2$, equivalently the elliptic
curve $E_6 : y^2 = x^3 - x$ with $\mathrm{rank}_{\mathbb{Q}}(E_6) = 0$
and only the four torsion points $(0, 0), (\pm 1, 0), \infty$, all
pulling back to $a c = 0$ or $a = c$ (Mordell, \emph{Diophantine Equations},
§17; Cassels, \emph{Lectures on Elliptic Curves}, Ch.~II).
\item $\mathcal{V}_{1/4}$ by rank-$0$ descent on its Jacobian elliptic
curve $E_{1/4} : y^2 = x(x-12)(x-16)$, Cremona \texttt{24a1}
(conductor $24$, rank $0$, torsion $\mathbb{Z}/4 \oplus \mathbb{Z}/2$).
\item $\mathcal{V}_{3/4}$ by rank-$0$ descent on its Jacobian elliptic
curve $E_{3/4} : y^2 = x(x-300)(x-1200)$, Cremona \texttt{72a2}
(conductor $72$, rank $0$, torsion $\mathbb{Z}/2 \oplus \mathbb{Z}/2$).
\end{itemize}
All torsion points correspond to the quartic-trivial cases $a c = 0$ or
$a = c$, both excluded by the pairwise-distinct hypothesis.

Combining faithfulness with the absence of closing relations,
$\rho : W_n \to \mathrm{Aff}(\mathbb{R}^n)$ is the faithful geometric
realisation of a right-angled Coxeter system, and
$u_d = [t^d]\, W_n(t)$.
\qed
\end{proof}
...
descents on Cremona \texttt{24a1} and \texttt{72a2} are verified in
SageMath via \texttt{EllipticCurve.rank()}.  The BFS values in
...
Each Lean theorem is checked by the kernel; building the projects
re-verifies every claim.
