...
affect $u_d$ unless $c_T \le 2d$), but whether it ever affects the
orbit counts at all remains open.
...
statement, which remains a conjecture.
...
$> 22$.  Whether this reverse inclusion holds for all $T$ is open.
...
shape's minimal polynomial $\Phi_T$ to divide a realized
word-difference, hence $c_T$ to divide its extreme coefficients, so
$c_T \le 2d$; whether
the obstruction ever materializes remains open, and we make no claim
that it must hold or that it must fail.
\end{remark}
...
\item Let $\Phi_T(t) = c_T\, t^2 - e_T\, t + c_T \in \mathbb{Z}[t]$
...
$u_e^T \neq u_e$ for some $e \le d$, then $\Phi_T$ divides a realized
...
representative is $\Phi_T(t) = c_T t^2 - e_T t + c_T$ with
$e_T = 2(a^2-b^2)$ when $a + b$ is odd and $e_T = a^2 - b^2$ when $a, b$
are both odd ($\Phi_T$ is primitive: an odd prime dividing $c_T$ and
...
$\widetilde D = \Phi_T \cdot E$ with $E \in \mathbb{Z}[t]$.
...
modular-arithmetic script rather than the Lean kernel.  We note two
structural facts visible in the same computations, recorded here as
data: through depth $16$, among all $666{,}913$ same-class difference
pairs the gcd of the two extreme coefficients contains a prime
$\equiv 1 \pmod 4$ in only $12$ cases, all of the form
$\mathrm{const}\cdot t^j (t-1)$ --- and $(t-1) \mid D$ holds for
\emph{every} difference, unconditionally, by the $P_w(1) = 0$ clause of
Lemma~\ref{lem:symbolic-form}.  Since $t - 1$ vanishes only at a root of
unity, Niven's theorem already excludes this factor from ever producing
a collision.  A proof that the extreme coefficients of all realized
differences avoid split primes (equivalently, that the dangerous factor
never strengthens beyond cyclotomic ones) would, by
Theorem~\ref{thm:effective-universality}, settle
Conjecture~\ref{conj:universality} in full; we leave this as the
sharpest currently visible route.
...
hence all-depths $T$-independence.  We do \emph{not} establish this:
$M = \mathbb{Z}[Q]/(Y+1, c+1)$ is free of rank $1$ over
$\mathbb{Z}[t^{\pm 1}]$ ($t = XY$), hence of infinite rank as an abelian
group, and Lemma~\ref{lem:dilation-invariance} only relates $\rho_{\lambda
T}$ to $\rho_T$ within a single similarity class; it does not compare
distinct shapes $b/a$ and does not force $\Phi_T$ injective.  The
all-depths statement is therefore left as
Conjecture~\ref{conj:universality}
(see Remark~\ref{rem:univ-conj-status}).
\qed
\end{proof}
...
is not machine-verified and remains open.
