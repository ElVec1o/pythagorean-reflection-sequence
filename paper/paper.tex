...
\emph{Equality $I_A \cap I_B = \mathbb{Z}[Q]\cdot h$.}  Membership and the
annihilator computation give a surjection of left $\mathbb{Z}[Q]$-modules
$\mathbb{Z}[Q]/(Y+1, c+1) \twoheadrightarrow \mathbb{Z}[Q]\cdot h
\subseteq I_A \cap I_B$, $1 \mapsto h$.  For the reverse inclusion, write
$Q = D_\infty \times \langle c\rangle$ with $D_\infty = \langle X, Y\rangle$
and let $t = XY$ generate the translation subgroup
...
the one-dimensional excess being spanned by the rotation-norm element
$\nu_N = \sum_{k=0}^{N-1} t^k$ (verified for $3 \le N \le 16$ in
...
$\mathbb{Z}[Q]$.  Hence the excess vanishes in $Q$, so
$I_A \cap I_B = \mathbb{Z}[Q]\cdot h \cong \mathbb{Z}[Q]/(Y+1, c+1) =: M$.
\qed
\end{proof}
...
realization of any triangle.  The explicit cyclic form
$M = \mathbb{Z}[Q]/(Y+1, c+1)$ established above is used only to write the
abstract realization $\rho_{\mathrm{abs}}$ down concretely
(Lemma~\ref{lem:rho-abs}); the universality conclusion itself does not
depend on it.
\end{remark}
...
Every $\rho_T$ has the same linear quotient $W \twoheadrightarrow Q$
(determined by the reflection pattern, not the leg lengths), so the
translation kernel $T_{\rho_T} = \ker(W \to Q)$ is the same subgroup of
$W$ for all $T$, and by Lemma~\ref{lem:trans-mod} its abelianization is
$T_{\rho_T}^{\mathrm{ab}} \cong I_A \cap I_B$ --- a purely
group-theoretic module independent of $T$
(Remark~\ref{rem:univ-robust}).  By Lemma~\ref{lem:rho-abs}
...
sending the generator $h$ to the translation $t(\rho_T(R_2))$.  By
Lemma~\ref{lem:dilation-invariance} $\Phi_T$ is \emph{injective} for
every nondegenerate $T$ (the image lattice has the same rank as $M$,
the dilation acting as a nonzero scalar on each $\mathbb{Z}[Q]$-isotypic
component), so
$\ker(\rho_T) = \ker(\rho_{\mathrm{abs}})$ for every $T$.  As the
right-hand side is independent of $T$, the orbit-growth sequence
$u_d^T = |\{\rho_T(w) : |w| = d\}|$ is constant in $T$.
\qed
\end{proof}
