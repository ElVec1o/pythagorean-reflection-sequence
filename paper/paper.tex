...
\begin{abstract}
For every right triangle $T$ in the plane with positive \textbf{unequal} rational legs, the BFS orbit-growth sequence $u_d^T$ under the group of side-reflections is $T$-independent, proven through depth $26$ and conjectured in general (Theorem~\ref{thm:universality}, Theorem~\ref{thm:universality-26}, Conjecture~\ref{conj:universality}); we call the common sequence the \emph{universal right-triangle reflection sequence}.  The translation kernel of the side-reflection representation is computed via Mayer--Vietoris and Crowell as the $\mathbb{Z}[Q]$-module $I_A \cap I_B$, conjecturally the cyclic ideal $\mathbb{Z}[Q]\cdot h$ on the central generator $h = (c-1)(Y-1)$ (established for the finite quotients $N \le 16$).  In every dimension $n \ge 2$, the right-corner orthoscheme reflection group has closed-form Coxeter--Steinberg growth rate $r_n = 1 + 2\cos(2\pi/(n+3))$ (Theorem~\ref{thm:ndim-rate}).  For pairwise distinct legs the affine representation is unconditionally faithful in every dimension $n \ge 3$ (Theorem~\ref{thm:master-C-uncond}), via a uniform Schur-complement determinant identity $\det Q_n = -\prod_i a_i^2$ (Lemma~\ref{lem:uniform-detQ}) together with rank-$0$ descent on the three Cremona curves $\mathtt{24a1}$, $\mathtt{32a2}$, and $\mathtt{72a2}$.  A dimension-independent collision-depth invariant $\mathrm{cd}_n(\lambda) \in \{\infty, 4, 3\}$ determined by the $(e, t)$-partition statistics of the leg sequence (Theorem~\ref{thm:cd-general}) classifies first-collision behaviour across all dimensions.  A triangle-independent symbolic normal form reduces extra collisions to the vanishing of bounded-height integer Laurent polynomials at the rotation number $\zeta_T = (a+bi)/(a-bi)$; the rational root theorem over $\mathbb{Z}[i]$ then yields an effective all-depths theorem --- every triangle whose reduced denominator norm $c_T$ exceeds $4d^2$ is universal at depth $d$ (Theorem~\ref{thm:effective-universality}) --- and, combined with a finite modular certificate over the remaining candidates, universality through depth $26$ for all unequal-leg rational triangles (Theorem~\ref{thm:universality-26}).  The eight length-$10$ relations, the Schur-complement identity, and the $T$-independence of the orbit-growth sequence through depth $22$ are formally machine-verified in the Lean~4 proof assistant (\S\ref{sec:reproduce}).  For the 2D universal sequence the consecutive ratios $u_{d+1}/u_d$ are empirically near $1.50$ at the computed depths (descending to $\approx 1.497$ at depth $42$); the exact asymptotic ratio $\beta_2$, and the algebraic nature of the generating function, remain open and are studied in the companion document \texttt{paper\_extra.tex}, together with the mod-$p$ state-space and Knuth--Bendix experiments.
\end{abstract}
...
affect $u_d$ unless $c_T \le 4d^2$), but whether it ever affects the
...
the proof above, through depth $26$ uniformly in $T$ by the arithmetic
...
reduced denominator norm $c_T$ to divide the squared extreme
coefficients of a realized word-difference, so $c_T \le 4d^2$; whether
...
\item If $u_e^T \neq u_e$ for some $e \le d$, then $c_T \le 4d^2$.
Consequently \emph{every} triangle with $c_T > 4d^2$ satisfies
universality at all depths $\le d$; equivalently, for every unequal-leg
rational triangle,
\[
u_d^T = u_d \quad\text{for all } d < \tfrac{1}{2}\sqrt{c_T}\, .
\]
\end{enumerate}
\end{theorem}
...
two ball elements of depth $\le d$; by
Lemma~\ref{lem:symbolic-form} its coefficients are bounded by $2d$ in
absolute value.  Normalize $\widetilde D = t^m D \in \mathbb{Z}[t]$ with
$\widetilde D(0) \neq 0$.  Since
$\widetilde D(\mu/\lambda) = 0$ with $\gcd(\mu, \lambda) = 1$, the
rational root theorem in $\mathbb{Z}[i]$ gives
$\lambda \mid \mathrm{lead}(\widetilde D)$ and
$\mu \mid \mathrm{const}(\widetilde D)$.  Taking norms,
$c_T = N(\lambda) \le |\mathrm{lead}(\widetilde D)|^2 \le (2d)^2$.
\end{proof}
...
reduces the conjecture to the finitely many triangles with
$c_T \le 4d^2$, and those can be checked mechanically.  Evaluating the
entire symbolic ball at each candidate $\zeta$ in the finite field
$\mathbb{F}_q$ (choosing $q \equiv 1 \pmod 4$ so that
$i \in \mathbb{F}_q$, and $q \nmid c_T$) is \emph{one-sided exact}:
images distinct mod $q$ are distinct in $\mathbb{Q}(i)$.  Running this
certificate at depth $26$ (ball of $636{,}805$ symbolic elements,
$862$ candidate rotation numbers with $c_T \le 2704$, two independent
$31$-bit primes; script \texttt{code/zeta\_probe/certify.py}, total
runtime about two minutes) found every candidate collision-free.
Combined with Theorem~\ref{thm:effective-universality}(ii) for
$c_T > 2704$ this proves:

\begin{theorem}[Universality through depth 26]
\label{thm:universality-26}
For every right triangle with positive unequal rational legs,
$u_d^T = u_d$ for all $0 \le d \le 26$.
\end{theorem}

Theorem~\ref{thm:universality-26} extends the kernel-checked Lean bound
of Remark~\ref{rem:lean-universality} by four further layers at
