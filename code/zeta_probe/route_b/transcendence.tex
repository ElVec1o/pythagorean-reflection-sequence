...
$\displaystyle\sum_{m\ge2}\sup_{u\in[0,w]}|L_m|=O(\tau)$. Indeed, expanding $R_j=\sum_{n\ge1}
\lambda_n\tau^{2n}P_n(j)$ ($P_n$ a polynomial of degree $2n{+}1$ with nonnegative coefficients,
$\lambda_n$ of alternating sign; the series converges for $j<\pi/\tau$, far beyond the effective
range $j\lesssim 2.3\,w$), the engine~\eqref{eq:engine} applies to each $\tau$-order, and the
triangle inequality over orders with~\eqref{eq:touchard} gives
$\sup_{[0,w]}|L_m|\le\tfrac{2}{m!}\,\mathbb E\bigl[\bar R(N/2)^m\bigr]$, $\bar R(j)=\sum_n|\lambda_n|
\tau^{2n}P_n(j)$. Hence $\sum_{m\ge2}\sup|L_m|\le 2\,\mathbb E\bigl[\Psi(\bar R(N/2))\bigr]$,
$\Psi(x)=e^x-1-x$, which is $O(\tau)$ \textup(bulk $N\le2w$: $\bar R(N/2)=O(\sqrt\tau)$ so
$\Psi\le\tfrac12\bar R^2 e^{\bar R}$; tail $N>2w$: Chernoff $\mathbb P(N>2w)\le e^{-w(2\ln2-1)}$\textup);
numerically $\le 0.0135\,\tau$ for $\tau\le0.02$, decreasing toward $\tau/648$.
\end{lemma}
...
at $\tau=10^{-4}$, $120$ digits\textup). The constituents are individually rigorous: the engine and the
Touchard identity~\eqref{eq:touchard} are exact, the per-$p$ bound and Poisson-moment identity are
elementary, the R-control~\eqref{eq:phibound} is proved, and $L_0,L_1$ with~\eqref{eq:L1bound} are
rigorous. \emph{No open mathematical obstruction remains}; the one item not yet written
symbol-for-symbol is the assembly bridge of Lemma~\ref{lem:tail} --- the Tonelli interchange of the
$\tau$-series (valid in its radius $\pi/\tau\gg2.3w$) and the Chernoff split bounding
$\mathbb E[\Psi(\bar R(N/2))]=O(\tau)$, both elementary and numerically corroborated with $5$--$15\times$
margin. Pending that write-up we state $V$ as transcendental modulo Lemma~\ref{lem:cos}; we emphasise
this is now a write-up step, not a mathematical gap.
