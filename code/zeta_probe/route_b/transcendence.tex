...
\]
This yields the central estimate, which we record as a separate lemma because it
is the one analytic input that we verify numerically rather than bound in closed
form (see Remark~\ref{rem:gap}).

\begin{lemma}[Cosine asymptotics; verified]\label{lem:cos}
...
$<1$ and values $>1$ at points arbitrarily close to $q=1$}. Lemma~\ref{lem:cos}
is a sharp, phase-uniform asymptotic that we have verified to high precision but
not bounded in closed form. The subtlety is genuine: the alternating terms $t_j$
reach magnitude $e^{(1+o(1))\sqrt{2/\tau}}$ near $j^{\ast}\sim\tfrac12\sqrt{2/\tau}$,
so the $O(\sqrt\tau)$ size of $S_1-(1-\cos w)$ is the result of near-total
cancellation, not of termwise smallness (indeed
$\sum_j|t_j-\hat t_j|$ is exponentially large). A fully rigorous proof of
\eqref{eq:cosasymp} is a standard but nontrivial saddle-point / Poisson-summation
estimate for the oscillatory sum $\sum_j(-1)^{j}t_j$: writing
$(-1)^{j}=e^{i\pi j}$ and applying Poisson summation, the $n=0,1$ frequencies
produce the $\cos w$ main term with the saddle at $j^{\ast}$, and the remaining
frequencies are exponentially subdominant. Carrying this out with explicit error
control (e.g.\ via the Euler--Maclaurin expansion of $\log t_j$ and a contour
deformation through the complex saddle) would upgrade the result to
\emph{unconditional}. Pending that estimate, the theorem is established modulo
Lemma~\ref{lem:cos}; the weaker statement actually used --- two sign-straddling
sequences --- is supported by the certification above at $58$ exhibited poles
...
and $d_n:=v_n-u_n$ growing only polynomially in $n$ (so $U,V$ share the
exponential rate). Transcendence of $U$ does \emph{not} follow formally from that
of $V$: in principle $U=V-D$ with $D(x)=\sum_n d_n x^{n}$ could be algebraic if
$D$ cancelled the poles of $V$. This is not expected --- $D$ is itself a
$q$-difference assembly with no algebraic recurrence in $33$ known terms --- but
proving transcendence of $U$ requires either (i) the same singularity analysis
for the connectivity-corrected kernel, or (ii) a proof that $D$ does not cancel
the infinitely many poles $\{1-\Sigma_1=0\}$. We therefore record:
\emph{$V$ is transcendental over $\mathbb Q(x)$ \textup(modulo
Lemma~\ref{lem:cos}\textup); the transcendence of the true series $U$ remains
conjectural.}
\end{remark}
