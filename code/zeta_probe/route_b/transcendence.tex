...
\paragraph{Scaling and the remaining lemma.} The layer norms obey
$\sup_{[0,w]}|L_m|=\Theta\!\bigl(\tau^{(m+1)/2}\bigr)$ \textup(measured: $L_2/\tau^{3/2}$ and
$L_3/\tau^{2}$ stable\textup), the top-$W$-power cancelling at every order as in $L_1$; in
particular the tail is summable. The engine majorant of the tail is bounded and
super-geometric in $m$ ($0.0097,0.0043,0.0025$ at successive $\tau$), whereas the naive
per-$j$ absolute majorant blows up ($3.8\times10^{15}$): the cancellation is genuinely
done by \eqref{eq:engine}, not deferred. The entire remaining content is:

\begin{lemma}[Tail majorization; one bookkeeping step]\label{lem:tail}
$\displaystyle\sum_{m\ge2}\sup_{u\in[0,w]}|L_m|=O(\tau^{3/2})$, via the engine bound
$\sup|L_m|\le\frac1{m!}\sum_p|c^{(m)}_p|\,P^{\mathrm{abs}}_p(w)$ \textup(the $c^{(m)}_p$ the
$j^p$-coefficients of $(-R_j)^m$, $\deg_j=3m$; $P^{\mathrm{abs}}_p(w)\le K(w/2)^p$ with $K$
bounded\textup), a Stirling/Cauchy estimate of degree-$3m$ coefficient growth against $1/m!$,
$(w/2)^p\le(2\tau)^{-p/2}$, and $\tau^{2m}$, uniform in $m$. \emph{This involves no oscillatory
cancellation} \textup(performed already by \eqref{eq:engine}\textup); it is the one step not
yet written symbol-for-symbol.
\end{lemma}

Granting Lemma~\ref{lem:tail}, \eqref{eq:L1bound} gives $\sup|\mathrm{corr}|\le\tfrac\tau6
+O(\tau^{3/2})$, hence by \eqref{eq:corrgoal} the bound $|T_2|=O(\sqrt\tau)$ and
Lemma~\ref{lem:cos}. Thus \emph{the hard analytic core of Lemma~\ref{lem:cos} is solved}
--- the oscillatory cancellation that defeated every soft route is carried out exactly by the
resummation engine, and $L_0,L_1$ together with their bounds are rigorous --- and closure of
the transcendence of $V$ awaits only the non-oscillatory majorization Lemma~\ref{lem:tail}.
We do not yet claim $V$ transcendental unconditionally on this basis.
