...
\begin{definition}[The explicit transducer $\mathcal T$]\label{def:transducer}
Read each side of the spine \emph{outward} from its endpoint, one edge at a time, classifying each
edge as a \emph{block} edge ($D$: an off-travel deposit) or a \emph{gap} edge ($G$: $a_j=0$), and
halting at the outermost block edge. The side automaton has four states $(\beta,s)$ ($\beta$: a block
has been seen; $s$: pending shield), starts in $(0,\mathrm{sh})$ with $\mathrm{sh}=\mathrm{sh}_R$
(right) or $\mathrm{sh}_L$ (left) of Proposition~\ref{prop:Tform}, and runs by
\[
...
\end{array}
\]
The output of $\mathcal T$ is the sum of the two side totals plus the boundary cell $\eta(g)$. By
construction a gap-run of length $L$ with initial shield $s\in\{0,1\}$ emits $\max(0,L-s)$, so
$\mathcal T$ realizes the closed form of Proposition~\ref{prop:Tform} on every profile.
\end{definition}

\begin{theorem}[The closed form via the explicit transducer]\label{thm:Bproved}
The output of $\mathcal T$ (Definition~\ref{def:transducer}) equals $c(g)=\tfrac12(\ell_T-\ell_R)$ for
\emph{every} $g$.
\end{theorem}
...
$275{,}823$-element depth-$24$ enumeration is such a window (held out at $19$--$24$). The explicit
transition list of $\mathcal T$ is now displayed (Definition~\ref{def:transducer}), so the covering
claim is checkable on paper; the transducer is additionally certified to reproduce the closed form on
$200{,}000$ random profiles with gap-runs and magnitudes far outside the window
(\texttt{fsm\_verify.py}), confirming the magnitude-independence of Lemma~\ref{lem:fsm}. This corrects
