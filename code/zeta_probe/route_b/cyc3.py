...
# fast relaxed length (closed form) from catalytic_funceq
...
dist=LL.bfs(maxd+2)   # need elements whose TRUE len <= maxd; relaxed<=true, BFS dist=true
vN=defaultdict(int); uN=defaultdict(int)
cyc=[]
for (e,dl,k,L),d in dist.items():
    tl=LL.solve(e,dl,k,L)       # true length (fast)
    rl=relaxed_len(e,dl,k,L)    # relaxed length (closed form)
    if tl is None or rl is None: continue
    if tl!=d:
        print("TRUE MISMATCH",(e,dl,k,dict(L)),"bfs",d,"solve",tl); continue
    if rl>tl:
