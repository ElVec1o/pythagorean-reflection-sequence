    ...
    for j in edges:
        ...
        for (comps,sp,ep),c in states.items():
            ...
            if len(comps)>=1:
                ...
                PROFILE_RECORDS.append((nd, maxmult, len(comps)))
...
if __name__=='__main__':
    ...
    # histogram of #distinct shapes per profile
    dd=Counter(r[0] for r in PROFILE_RECORDS)
    print("\nHistogram of #distinct-shapes per profile:")
    for k2 in sorted(dd): print(f"   {k2} distinct shapes : {dd[k2]} profiles")
    # relationship: total comps vs distinct shapes
    print("\nFor profiles with many components, is it ~1 repeated bulk shape?")
    big=[r for r in PROFILE_RECORDS if r[2]>=4]
    if big:
        print(f"   {len(big)} profiles with >=4 components")
        print(f"   among these, max #distinct shapes = {max(r[0] for r in big)}")
        print(f"   (so high count = high multiplicity of few shapes)")
