...
/// Sphere sizes of the group generated by `gens`, by breadth-first search in the Cayley
/// graph.  Elements are affine maps over F_p, so equality is exact.  Bounded by MAX_BALL.
const MAX_BALL: usize = 4_000_000;

fn spheres(gens: &[Aff; 3], depth: usize) -> Option<Vec<usize>> {
...
fn main() {
    ...
    // ---- sphere counts: where does each stratum first fall below the generic growth? ----
    {
        const D: usize = 14;
        let ggens = generic_gens(2, 7, 3, 5);
        let gen_sph = spheres(&ggens, D).expect("generic ball cap");
        println!("=== sphere counts to depth {} ===", D);
        println!("  generic: {:?}", gen_sph);
        for m in [3u64, 4, 6, 8, 9, 10, 11, 12] {
            let zeta = powm(g, (prime - 1) / (2 * m));
            for (a, b) in [(2i64, 3i64), (1, 3)] {
                let sg = stratum_gens(m, a, b, zeta, i_unit);
                if let Some(sp) = spheres(&sg, D) {
                    let first = (1..=D).find(|&d| sp[d] != gen_sph[d]);
                    let deficits: Vec<i64> = (1..=D)
                        .map(|d| gen_sph[d] as i64 - sp[d] as i64)
                        .collect();
                    println!(
                        "  m={:2} legs {}:{}  first difference at depth {:?}   deficits(1..{})={:?}",
                        m, a, b, first, D, deficits
                    );
                }
            }
        }
