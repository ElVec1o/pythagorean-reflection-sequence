...
# ---------------- saddle heights ----------------
x0 = C(iv.log(4 + iv.sqrt(15)) / 4, 3 * PI / 8)
xm = C(x0.re, -P8)
F0 = Phi(x0)
Fm = Phi(xm) - C(0, PI) * xm
T0 = rot(F0); Tm = rot(Fm)
T_LO = min(lo(T0), lo(Tm)); T_HI = max(hi(T0), hi(Tm))
...
print('T0 in [%.8f, %.8f],  Tm in [%.8f, %.8f];  T_LO = %.8f' % (lo(T0), hi(T0), lo(Tm), hi(Tm), T_LO))
...
def certify(name, xfun, dxdu, fval, fder, n, u0, u1, ncell=32, target=TARGET):
    ...
    print('%-58s n=%+d cells=%5d  sup - T_LO <= %+.5f' % (name, n, cells, worst), flush=True)
    results[name + ' n=%+d' % n] = worst
    return worst
...
# P1: start box B_c (contains x_c for both branches, all connectors), all four exponents
certify_box('B_c start box (crossing of the negative s-axis)', BC_RE, BC_IM, [0, -1, -2, 1])

# P2: Pi_0 vertical strip, Re x in [xc*-EPS, xc*+EPS], Im x from -pi/8-EPS to 3pi/8  (U, n=0)
y_lo = lo(-P8) - EPS; y_hi = hi(3 * P8)
certify('Pi_0 vertical (crosses the negative real x-axis)', lambda u: C(BC_RE, u), 1, U_hi, dU_hi, 0, y_lo, y_hi)

# P3: Pi_0 horizontal Im x = 3pi/8:  U on [xc*-EPS, 0], W on [0, x0-DELTA] and [x0+DELTA, X_T]
Y3 = 3 * P8
certify('Pi_0 horiz Im=3pi/8, Re<=0 (reflection bound)', lambda u: C(u, Y3), 1, U_hi, dU_hi, 0, XC_RE - EPS, mpf(0))
x0r_lo = lo(x0.re) - DELTA; x0r_hi = hi(x0.re) + DELTA
certify('Pi_0 horiz Im=3pi/8, 0<=Re<=x0-DELTA', lambda u: C(u, Y3), 1, W_hi, dW_hi, 0, mpf(0), x0r_lo)
certify('Pi_0 horiz Im=3pi/8, x0+DELTA<=Re<=X_T', lambda u: C(u, Y3), 1, W_hi, dW_hi, 0, x0r_hi, X_T)

# P4: G- horizontal Im x = -pi/8: n=-1 (main, disk at xm) and n=-2 (remainder R_-)
Y4 = -P8
certify('G- horiz Im=-pi/8, Re<=0 (reflection bound)', lambda u: C(u, Y4), 1, U_hi, dU_hi, -1, XC_RE - EPS, mpf(0))
certify('G- horiz Im=-pi/8, 0<=Re<=xm-DELTA', lambda u: C(u, Y4), 1, W_hi, dW_hi, -1, mpf(0), x0r_lo)
certify('G- horiz Im=-pi/8, xm+DELTA<=Re<=X_T', lambda u: C(u, Y4), 1, W_hi, dW_hi, -1, x0r_hi, X_T)
certify('G- horiz Im=-pi/8, Re<=0 (reflection bound)', lambda u: C(u, Y4), 1, U_hi, dU_hi, -2, XC_RE - EPS, mpf(0))
certify('G- horiz Im=-pi/8, 0<=Re<=X_T', lambda u: C(u, Y4), 1, W_hi, dW_hi, -2, mpf(0), X_T)

# P5: G+ segment x_c -> i pi/8  (x_c ranges over B_c), n=+1, U
XCB = C(BC_RE, BC_IM)
DIR5 = C(0, P8) - XCB
# the union over x_c in B_c of the segments x_c(1-u) + i pi/8 u is covered by the boxes
# (1-u)B_c + i pi/8 u, u in [k/NU, (k+1)/NU]; each box is certified in 2-D (U, n=+1)
NU = 48
w5 = mpf(-100)
for k in range(NU):
    ...
    certify_box('_seg', rer, Xk.im, [1], quiet=True)
    w5 = max(w5, results.pop('_seg n=+1'))
print('%-58s n=+1 boxes=%5d  sup - T_LO <= %+.5f' % ('G+ segment x_c -> i pi/8 (reflection bound, 2-D)', NU, w5), flush=True)
results['G+ segment n=+1'] = w5
# P6: G+ ray i pi/8 + rho e^{i psi0}, 0 <= rho <= RHO_T, n=+1, W
E6 = C(iv.cos(PSI0), iv.sin(PSI0))
certify('G+ ray i pi/8 + rho e^{i 55deg}, rho<=RHO_T', lambda u: C(0, P8) + E6 * u, 1, W_hi, dW_hi, 1, mpf(0), RHO_T)
...
conc('disk |u|<=DELTA around x0 (n=0)', x0)
conc('disk |u|<=DELTA around xm (n=-1)', xm)
...
def tail_h(name, y0, n):
    ...
    print('%-58s n=%+d  Wt - T_LO <= %+.4f, slope <= %+.4f' % (name, n, val, slope), flush=True)
    results['tail ' + name + ' n=%+d' % n] = val
    return val < 0 and slope < 0


def tail_ray():
    x = C(0, P8) + E6 * RHO_T
    val = hi(rot(x * (LW + C(0, PI)) - x * x + PI ** 2 / 96)) + tau(lo(x.re)) - T_LO
    slope = hi(rot((LW + C(0, PI) - C(0, P8 * 2)) * E6) - iv.mpf(2 * RHO_T) * iv.cos(2 * PSI0 - TH)) + SLACK_LOW
    ok = lo(iv.cos(2 * PSI0 - TH)) > 0
    print('%-58s n=+1  Wt - T_LO <= %+.4f, slope <= %+.4f' % ('tail G+ ray rho>=RHO_T', val, slope), flush=True)
    results['tail G+ ray n=+1'] = val
    return ok and val < 0 and slope < 0


tails_ok = all([tail_h('tail Pi_0 Im=3pi/8, Re>=X_T', 3 * P8, 0), tail_h('tail G- Im=-pi/8, Re>=X_T', -P8, -1),
                tail_h('tail G- Im=-pi/8, Re>=X_T', -P8, -2), tail_ray()])
...
lemma_scan(lambda u: C(u, Y3), XC_RE - EPS, mpf(0), True)
...
LAM = max(sups['Lam_i'], sups['Lam_ii']); KK = max(sups['K_i'], sups['K_ii'])
LEN = mpf(40)
