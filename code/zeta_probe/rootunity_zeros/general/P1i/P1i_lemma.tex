...
\item Dominant mode: $\Pi_\mu=[x_A,P_\mu]\cup(x_\mu+e^{i\theta/2}[-\lambda_\mu,\infty))$ with $P_\mu=x_\mu-\lambda_\mu e^{i\theta/2}$ and $\lambda_\mu=\operatorname{Re}x_\mu/(2\cos\frac\theta2)$. So the path crosses the saddle on its steepest-descent line.
 This change of path from P1f is needed at small $N$. At $\frac13$ the straight segment $[x_A,x_\mu]$ meets the saddle at $80.5^\circ$ from the steepest-descent direction, within $9.5^\circ$ of a level direction. $\Omega_\mu$ is not entire: it has a logarithmic/dilogarithm branch point from $\operatorname{Li}_2(e^{-2Nx})$ at $e^{-2Nx}=1$, i.e.\ on $\operatorname{Re}x=0$. Condition (d) of Proposition~\ref{prop:cert} certifies $\operatorname{Re}x>0$ on every box and ray of both the old and the new path, so $\Omega_\mu$ is holomorphic on the simply-connected half-plane containing both paths, and Cauchy's theorem (not global entireness) gives the same integral for any path with the same endpoints there.
...
\item \textbf{Path change.} The dominant paths differ from P1f (descent-line polygon). This is legitimate because $\Omega_\mu$ is holomorphic on the half-plane $\operatorname{Re}x>0$ that condition (d) certifies both paths lie in, not because it is entire; the deformation is Cauchy's theorem on that half-plane. Reviewed by Reviewer AE (CORRECT, with this wording fixed).
