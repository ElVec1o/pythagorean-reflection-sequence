...
\title{P1f: zeros of $B$ at every root of unity of the middle arc,\\ density of the poles of $G^T_0$, and the transfer to $U$ and $V$\\ \small research programme P1, sixth atom, 2026-09-24. UNREVIEWED.}
\date{}
...
For $N\ge16$ and $0\le k\le K$: $r\log|b_k|\le H_{\rm head}+r\log(1/P_N)$ with $H_{\rm head}=\delta(\ell+0.0837+r)+\frac{2\delta}N(1+\log\frac1{1.8\delta})\le0.0167+r$, where $P_N>0$ is the least partial product $\prod_{i\le J,\,N\nmid i}0.959|1-\zeta^i|$ over one period.
\end{lemma}
\begin{proof}
$|q^{k^2}|\le1$; $|c_t|\le|c|+2r(1.01)$, so $\log|c_t|\le\ell+r$. For $i\le2k\le2\delta/r$, $N\nmid i$: $|1-e^{-it}|\le\frac1{4N}e^{1/64}\le\frac{0.254}N$ and $|1-\zeta^i|\ge2\sin\frac\pi N\ge\frac{6.24}N$, so $|1-\zeta^ie^{-it}|\ge0.959|1-\zeta^i|$; full periods contribute $N\ge1$. For $i=Nm$: $|1-e^{-Nmt}|\ge0.99Nmr$. With $M'=\lfloor2k/N\rfloor\le2\delta/(Nr)$ and $\log M'!\ge M'\log M'-M'$, the function $M'(1+\log\frac1{0.99NrM'})$ is increasing on the range, which gives the second term. Numerically at $N=16$: $0.01148+0.00514$; both terms decrease in $N$.
\end{proof}
...
\begin{enumerate}
\item[(a)] For $\mu\in\mathcal M\setminus\{n,n+s\}$ with $|\mu|\le4N$: $W_\mu\le T-\eta_N$ on $\Pi_\mu$, $\eta_N=\min(16.2/N^2,\,0.1)$.
...
(ii) $S_-/S_e=\frac{A^-}{A^+}$ at the dominant mode $\mu$ ($\mu=n$ or $n+1$) $=e^{2x_\mu}\frac{G^*(\mu+a)}{G^*(\mu-a)}\frac{Z^-_N}{Z^+_N}$. For $N$ odd $\frac{G^*(\mu+a)}{G^*(\mu-a)}=\e(-\mu/N)$; for $4\mid N$ the same from $G^*(2m')=\e(-\bar am'^2/N)G^*(0)$; for $N\equiv2\ (4)$ likewise by the CRT factorisation of Theorem~2 of P1d. And $e^{2x_\mu}=c\,\e(\mu/N)(1-w)^{-2/N}=\e(\mu/N)/u_0$. Hence $S_-/S_e\to\omega$.
