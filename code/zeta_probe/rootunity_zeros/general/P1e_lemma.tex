...
Notation and numbering of P1d\_lemma.tex are used throughout: Theorem~1 (Gauss factorisation $\Sigma_N=G^*_Ne^{-\Phi(\varepsilon)}Z_N$), Theorem~3 (Fresnel representation), Lemmas W, conj, dec, lap, Definition main ($\mathrm{Main}'_Z$, $S_{\rm amp}$, $t_0$, $F$, $L$, $B_k$, $\gamma_k$).
...
\item[(4)] If $x$ lies in no hazard with $n\ge10$ but in a low-seed disc of $p/q$ (radius $r\le1/120$, $n_2=60$), then $\Delta_N(x;\rho_\eta)\le\tau/60<10^{-3}$. So the $\Delta$-hypotheses of Theorem 5 of P1d hold.
\end{enumerate}
\end{lemma}
\begin{proof}
...
\end{itemize}
In (2), a shallower hazard $i'/m'$ ($m'<n$) also containing $x$ satisfies $|x-i'/m'|\ge1/(2nm')$, because $nm'\varepsilon_n\le\frac12$. That is incompatible with $y^m/(4\tau)$ for $m>n_2$.
...
\end{lemma}
\begin{proof}
Apply Lemma lap (P1d) with $v_0=\eta/2$, $t_0$ the unique zero of $F'$ in $|t|\le\eta$ (a contraction: $|t_0|\le R_0=\bar w/((1-\bar w)\pi n)$, $\bar w=(g\rho)^n\le y^n$), and the following bounds valid on $\operatorname{Im}t\le\eta$.
\begin{itemize}
\item $|W|\le\bar w$.
...
1/2 & 0.0083 & 0.102 & 0.4944 & 0.000270 & 1.144 & 0.5468 \\
1/3 & 0.004 & 0.087 & 0.2927 & 0.00131 & 1.089 & 0.7689 \\
2/3 & 0.003 & 0.055 & 0.6472 & 0 & 1.093 & 0.3847 \\
1/4 & 0.003 & 0.055 & 0.7730 & 9.79e-6 & 1.045 & 0.2409 \\
3/4 & 0.0007 & 0.0257 & 0.8204 & 2.09e-6 & 1.054 & 0.1924 \\
1/5 & 0.0005 & 0.0257 & 0.4907 & 0 & 0.9681 & 0.4928 \\
2/5 & 0.003 & 0.102 & 0.05721 & 4.10e-5 & 1.086 & 1.024 \\
3/5 & 0.003 & 0.087 & 0.06302 & 0 & 1.087 & 1.018 \\
2/7 & 0.002 & 0.055 & 0.3018 & 0 & 1.077 & 0.7519 \\
5/7 & 0.002 & 0.055 & 0.3032 & 8.21e-6 & 1.078 & 0.7512 \\
3/7 & 0.002 & 0.102 & 0.02727 & 1.69e-6 & 1.074 & 1.045 \\
4/7 & 0.002 & 0.102 & 0.02694 & 0 & 1.076 & 1.047 \\
3/8 & 0.002 & 0.087 & 0.02048 & 9.70e-9 & 1.095 & 1.073 \\
5/8 & 0.002 & 0.087 & 0.01971 & 1.01e-8 & 1.097 & 1.075 \\
2/9 & 0.0005 & 0.0257 & 0.5568 & 3.62e-6 & 1.019 & 0.4516 \\
7/9 & 0.0005 & 0.0257 & 0.5579 & 0 & 1.019 & 0.4505 \\
4/9 & 0.0015 & 0.102 & 0.02222 & 7.85e-8 & 1.068 & 1.044 \\
5/9 & 0.002 & 0.102 & 0.03171 & 0 & 1.068 & 1.034 \\\bottomrule
\end{tabular}\end{table}
(The $|S_{\rm amp}|$ and $c_{\rm seed}$ columns are parsed from 4-digit printouts of Arb lower bounds. The margin over the needed $c_0=0.02$ is a factor of at least 9.)
...
\emph{Base} $N\le150$. \texttt{P1e\_logderiv.py scan} encloses $Z_N(a/N)$ by Lemma~\ref{lem:Zk} for all $N\le150$ and $\ell\ge0.8546$, giving $|Z_N|\ge0.9135$. (Nonvanishing of $\Sigma_N$ for $N\le150$ is also P1b's interval certificate.)
...
\item \emph{$x$ lies in no hazard with $n\ge10$ but in a low-seed disc.} Lemma~\ref{lem:H}(4) and Theorem 5 of P1d (the certificate of Table~\ref{tab:low}) give $|Z_N|\ge c_{\rm seed}e^{-\kappa_{\rm seed}N}\ge c_0e^{-\kappa N}$.
\item \emph{Otherwise.} \texttt{P1e\_caseA.py} covers $[\frac15,\frac12]$ minus all hazards of height $\le59$ and the low-seed discs, by 22,546 Arb intervals with 0 failures. It shows $\sum_{m<60}\sup|a_m|+\tau/59\le0.6057$. With Lemma~\ref{lem:H}(3) and Lemma~\ref{lem:A} this gives $|Z_N|\ge2-e^{0.6057}=0.167$.
\end{enumerate}
The cases are exhaustive. Theorem 1 of P1d converts the result to $\Sigma_N$, and S0\_lemma converts it to $S_0$.
\end{proof}
...
An argument-principle proof from Theorem 3 was not found; the integrand $e^{\Phi}$ has no positivity. (HEURISTIC assessment.)
\end{remark}
...
\paragraph{Files.} \texttt{P1e\_renorm.py} (Lemma R), \texttt{P1e\_logderiv.py} (Lemma Zk enclosures; $\Lambda,D_1,D_2$; base scan), \texttt{P1e\_cert.py}, \texttt{P1e\_all.py} (Lemma O certificate and search), \texttt{P1e\_lowseed.py} (Table~\ref{tab:low}), \texttt{P1e\_lemmaB.py} (Lemma B), \texttt{P1e\_lemmaBp.py} (Lemma B$'$), \texttt{P1e\_caseA.py} (case A covering), \texttt{P1e\_certlog.txt} (raw outputs).
