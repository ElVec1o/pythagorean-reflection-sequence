...
language, made explicit here); (ii) \textbf{correction (post-review, Reviewer AO): Lemma~\ref{lem:infinite-carry}
is NOT new.} It reproves, via an independent $p$-adic argument, the non-finite-generation of
$R=A_T=\Z[\zeta_T^{\pm1}]$ already established in \texttt{paper1.tex}'s Proposition ``Structure of
$W_T$'', part~(i) (a three-line B\'ezout argument: $e/c=\zeta+\zeta^{-1}\in A_T$ with $\gcd(c,e)=1$
gives $1/c\in A_T$, hence $A_T\supseteq\Z[1/c]$, not finitely generated since $c\ge2$) -- a strictly
simpler proof of the same fact, using the same object under a different name, that the original
version of this note failed to cross-check against. It is kept below only because its explicit
denominator-growth bookkeeping makes the connection to Gap 2's carry-boundedness question concrete,
not as new mathematics; (iii) a worked one-sided sub-case where the catalytic/kernel
...
\begin{lemma}[Infinite carry-class group -- NOT NEW, see correction above]\label{lem:infinite-carry}
...
$(2,1),(5,-6),(53,-90)$, all $|c_T|\ge2$). Then $R=\Z[\zeta^{\pm1}]$ is not finitely generated as a
$\Z$-module (hence, in particular, is an infinite abelian group; the converse implication is not
...
\item \textbf{Overall status: Conjecture~\texttt{conj:WT-nonDfinite} remains OPEN.} This note's only
genuine new content is the cardinality diagnosis of \S\ref{sec:obstruction} (a HEURISTIC sharpening of
Gap 3 from ``no equation is known'' to ``a specific, nameable obstruction blocks the standard
technique'') and the worked kernel-method sub-case of \S\ref{sec:kernel}. Lemma~\ref{lem:infinite-carry}
is, per the correction above, a reproof of an already-published fact (paper1.tex's \texttt{A\_T}
non-finite-generation), not new mathematics. Nothing here closes P5.
\end{enumerate}
