...
\textbf{New result (PROVED given \texttt{cor:arcUV}):} for every $A>0$,
\[
   n(r,U)\ \ge\ \frac{A}{1-r}\qquad\text{for all $r$ sufficiently close to $1$},
\]
i.e.\ $\displaystyle\liminf_{r\to1^-}(1-r)\,n(r,U)=+\infty$: the pole-counting function of $U$ grows
\emph{strictly faster than any constant multiple of $1/(1-r)$}. This sharpens, and in one respect
corrects, the back-of-envelope guess in P4v1 \S5(a) (which guessed a $\log(1/(1-r))$-type count near
each root of unity, times $\sim1/(1-r)$ many roots of unity): the correct per-root-of-unity count is
itself \emph{linear} in $1/(1-r)$, not logarithmic, and a super-any-fixed-multiple bound already
follows from a two-line diagonal argument that needs no uniform control over the certificate
constants across roots of unity. \textbf{What this does not do:} it stops short of a super-polynomial
bound $n(r,U)\ge (1-r)^{-\rho}$ for every fixed $\rho$, which is what would be needed to beat every
fixed-order/degree Bank--Kaufman-type bound at once (ingredient (c) of P4v1); that stronger statement
needs uniform-in-$N$ control of thresholds and constants that paper2a explicitly does not make
explicit (\texttt{rem:arcUV}). \S\ref{sec:diagnosis} pins down exactly what extra input would close
that gap. Ingredient (b) (the disc-analogue counting theorem itself) is untouched by this note.
