...
of P1f). Corollary~\texttt{cor:arcUV} now covers $\frac aN\in[\frac16,\frac56]$ (after P1h); the proof only
needs some infinite family of odd-order accumulation points, and $\frac aN\in[\frac15,\frac45]$, for example
$a=\frac{N-1}2$, already suffices, so this update does not weaken anything. Nothing else is
...
\end{lemma}
\begin{proof}
Embed $K(w,\lambda)$ in $\overline{K_{\mathfrak p}}$. First, $v(w)=0$: from $w^2-(2+X)w+1=0$ we get
$w+1/w=2+X$, and $v(2+X)=1$ since $v(X)=N\ge5>1=v(2)$. If $v(w)\ne0$ then $w$ and $1/w$ have valuations
of opposite sign (as $v(w)+v(1/w)=0$), so $v(w+1/w)=\min(v(w),v(1/w))<0\ne1$, a contradiction; hence
$v(w)=0$ exactly. Now write $w=1-\delta$. From $(w-1)^2=Xw$ we get $v(\delta^2)=v(X)+v(w)=N$, so $v(\delta)=N/2$.
...
\texttt{P3\_field.py} and \texttt{P3\_degree.py} take the truncation order $K$ as an explicit argument; since
$|u_0|\approx0.27$--$0.29$, the defining series converges slowly and $K\gtrsim15N$ is needed for reliable PSLQ
convergence at the working precision. Both scripts now warn on the command line when $K<15N$, so a rerun with
too small a $K$ (e.g.\ the $K=30$--$60$ used in some early exploratory runs) is flagged rather than silently
