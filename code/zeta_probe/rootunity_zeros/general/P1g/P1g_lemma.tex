...
\title{P1g: the hazard induction for $Z^\pm_N$ and the $\omega$-conditions of Theorem C\\ at every root of unity of the middle arc\\ \small research programme P1, seventh atom, 2026-09-24. Reviewed adversarially (Reviewer~AC); all gaps fixed, all certificates rerun.}
\date{}
...
with $\delta_R$ exactly as in P1e Lemma~R. In particular $|S^s|\ge e^{-\pi|\operatorname{Im}t_0|}(1-R^s)|T^s|$, where $R^s:=\mathrm{env}^s(e^{\delta_R}-1)/|T^s|$.
\end{lemma}
\begin{proof}
...
For each of the 18 low seeds of P1e (side $d>0$; the side $d<0$ of $p/q$ is the side $d>0$ of $(q-p)/q$), P1e's parameters $r,\eta,V_0$, $n_2=60$ and $\kappa=3$, and 8 geometric pieces, \texttt{P1g\_lowseed.py} certifies (it exits~1 if the certified $r'$ falls below the tabulated value, which is passed as a target; all 18 pass, \texttt{run\_lowseeds.sh}), on every piece of $(0,r']$ with $r'$ as in Table~\ref{tab:L}:
\begin{itemize}
...
(Correction, Reviewer~AC: the script previously excluded the hazard discs of height $53\le n\le59$ with radius $25\cdot2^{-n}$, twice $y^n/(4\tau)=12.5\cdot2^{-n}$, so the annuli $12.5\cdot2^{-n}\le|x-j/n|<25\cdot2^{-n}$ were not covered. Fixed to $25\cdot2^{-(n+1)}$ and all four blocks rerun on 2026-09-24; the interval count and all constants below are unchanged.)
...
\item[(iii)] \emph{Tail $n>6000$ (by hand, not machine-checked; as in P1e).} Here $\bar w,R_0\le2^{-n}$, $d\le12.5\cdot2^{-n}$ and $n_2\ge2^n/(25n)-1$. Also $\beta e^{\pi\eta}\le1.4\cdot2^{n/2}$, $P_1+\pi\le4n$ and $P_2\le40n$. So $r_1+r_3\le(400n^2+3n+110)2^{-n/2}$, while $r_2,r_4,r_5\le\exp(-10^{-5}2^{n})$. This gives $E\le10^5n^{5/2}(e^{0.005}/\sqrt2)^n<10^{-860}$, and $R\le10^4n^{5/2}(e^{0.005}/\sqrt2)^n<10^{-860}$ as in P1e.
 Further, $\epsilon_u\le160\cdot2^{-n}$. Put $\theta:=e^{0.005}/\sqrt2=0.71066$. Since $E,R<10^{-860}$, $\frac{(1+R)(1+E)}{(1-R)(1-E)}\le1+2.001(E+R)$, so by Lemma~\ref{lem:I}
 \[\rho\le2\epsilon_u+4.02(E+R)\le320\cdot2^{-n}+4.43\cdot10^5n^{5/2}\theta^n\le4.5\cdot10^5\,n^{5/2}\theta^n,\]
 using $2^{-n}\le10^{-600}n^{5/2}\theta^n$ for $n>6000$ (as $2\theta>1.42$). With $|\omega_q|\le10$ and $|d|\le12.5\cdot2^{-n}$,
 \[\sigma(n)\le15\rho+12.5\cdot2^{-n}\big(\tfrac{33\pi}2+4\pi\big)\le7\cdot10^6\,n^{5/2}\theta^n .\]
 The ratio of consecutive terms is $\le\theta(1+\frac1{6000})^{5/2}<0.7110$, so $\sum_{n>6000}\sigma(n)\le7\cdot10^6\cdot6001^{5/2}\theta^{6001}/0.289<10^{-870}$, and likewise $\sum_{n>6000}\rho(n)<10^{-871}$.
 (Correction, Reviewer~AC: an earlier version stated $\rho\le500\cdot2^{-n}$ and $\sigma(n)\le9000\cdot2^{-n}$, which ignored the $E,R$ contributions to $\rho$; those decay only like $n^{5/2}\theta^n$. The conclusion $S\le4.97\cdot10^{-12}$ is unaffected.)
\end{enumerate}
\end{lemma}
...
\end{corollary}
\begin{proof}
P1f Theorem~C(i)--(iii) with Theorem~\ref{thm:main}, and P1e Theorem~M for $Z_N\ne0$ on $[\frac15,\frac45]$ (reference added at the request of Reviewer~AC). For the arcs: the accumulation set is closed and contains $\pm\e(\frac a{2N})$ and $\pm\e(-\frac a{2N})$ for all admissible $a/N$, $N\ge16$, and these points are dense in the two arcs.
\end{proof}
...
\begin{enumerate}
\item \textbf{Adaptations (reviewed).} Lemma~\ref{lem:lap} is a transcription of P1d Lemma~8 and P1e Lemma~O with a bounded analytic factor inserted. The only genuinely new estimate is the lower-ray tail (halved Gaussian). Both scripts are line-by-line edits of the reviewed \texttt{P1e\_cert.py} and \texttt{P1e\_lemmaB.py}, and were checked by Reviewer~AC; the gaps found are corrected in place above and all certificates were rerun.
