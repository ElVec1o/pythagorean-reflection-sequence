...
\small research programme P1, eighth atom, 2026-09-24. Reviewed by Reviewer AD (judged correct with minor gaps; gaps addressed 2026-09-24).}
\date{}
...
\item \emph{Low seeds} (\texttt{P1e\_lowseed.py}, \texttt{P1h\_lowseed.py}; P1d Theorem 9 with P1e Lemma O; side $d>0$):
 ...
 The last column is the $\Delta$-hypothesis of P1e Lemma H(4) with the region base. For $5/6$ it holds only on $r'=2.5\cdot10^{-4}$, not on the full certified radius $5\cdot10^{-4}$. So the seed $5/6$ (i.e.\ $\frac16$ from the left) is used \emph{only} on the disc of radius $r'=2.5\cdot10^{-4}$, in Theorem~M$'$ as well as in Theorem main$'$. The covering disc $(\frac16-0.999\cdot2.5\cdot10^{-4},\frac16)$ of item~5 is chosen accordingly.
...
\item \emph{Deep seeds} (\texttt{P1h\_lemmaB.py crude}). P1e Lemma B holds with the region data of Table~\ref{tab:reg} and the induction hypothesis $|Z_n|\ge0.02e^{-0.005n}$:
 ...
 \item on $R_1$ for $301\le n\le2\cdot10^5$, with $\max E\le0.438$ (at $n=301$), $R\le0.125$. The script also prints $\log E$ (Arb upper bound, no underflow): $\log E(3000)\le-203.41$, so $E(3000)\le4.56\cdot10^{-89}$ (\texttt{lemmaB\_R1\_crude.txt}), and $\log E(2\cdot10^5)\le-15388.5$, so $E(2\cdot10^5)\le e^{-15388}<10^{-6682}$ (\texttt{lemmaB\_R1\_crude\_20001\_200000.txt}, regenerated with $P1H\_Y=0.59$, $P1H\_G=0.5762$, $P1H\_ETA=0.0037$, which the PASSED line now prints).
 \end{itemize}
...
\item \emph{Low-seed disc, no hazard.} Use item~2 (for $5/6$ only on $r'=2.5\cdot10^{-4}$): $|Z_N|\ge0.452\,e^{-4.5\cdot10^{-4}N}$.
...
\end{itemize}
In detail, each such margin is $Q(\ell)=\frac{\ell^2}4-\frac{\ell}{4N}+C$ with $C$ independent of $\ell$, and for $\ell\ge\ell_1$
\[
Q(\ell)-Q(\ell_1)=(\ell-\ell_1)\Big(\frac{\ell+\ell_1}{4}-\frac1{4N}\Big)\ \ge\ (\ell-\ell_1)\Big(\frac{\ell_1}{2}-\frac1{4N}\Big)\ \ge0,
\qquad\text{since }\ell_1\ge0.55>\tfrac1{2N}\ (N\ge16).
\]
Likewise $\operatorname{Re}y\ge\ell/4\ge\ell_1/4$ gives $1/(e^{2N\operatorname{Re}y}-1)\le1/(e^{N\ell_1/2}-1)$.
...
\texttt{P1h\_landscape.py} evaluates all of them in Arb for $N_1\le N\le10^5$ (all positive). For $N>10^5$ the argument is by hand. Each height margin is its limit $\frac{\ell^2}4\ge\frac{\ell_1^2}4$ minus the $N$-dependent corrections $\frac{\ell}{4N}$, $\frac{\pi^2}{6N^2}$ and the head term $\frac{2\delta}N\log\frac1{1.8\delta}$ ($\delta=\frac1{8N}$). For $N>10^5$ and $\ell\le\log4$:
\[
\frac{\ell}{4N}+\frac{\pi^2}{6N^2}+\frac{\log(8N/1.8)}{4N^2}\ \le\ 3.5\cdot10^{-6}+1.7\cdot10^{-10}+3.4\cdot10^{-10}\ <\ \frac{0.55^2}{8}\ \le\ \frac{\ell_1^2}8,
\]
so every height margin is $\ge\ell_1^2/8>0$. For the $N^{-2}$ margins, $|\tan\theta^*|\le\pi s/(N\ell_1)\le\pi s/(0.55\cdot10^5)$ gives $\cos\theta^*\ge0.99$ (for the $s$ used), and
\[
\frac{2\pi^2s^2\cos\theta^*-C_R}{N^2}\ \ge\ \frac{2\pi^2\cdot0.99-1.65}{N^2}\ >\ \frac{17.8}{N^2}\ >0\qquad(s\ge1).
\]
...
\begin{enumerate}
\item \textbf{Review scope.} Reviewer AD judged the note correct with minor gaps (addressed here). Not re-derived independently: Lemma~\ref{lem:Hy} (region-dependent bases), Proposition~\ref{prop:A} ($M>59$), and the transcription of the P1f landscape estimates into \texttt{P1h\_landscape.py} were checked by one reviewer, not re-derived line by line. The landscape script re-derives the P1f bounds as functions of $(\ell,N)$; an error there would affect Theorem B$'$ only, not Theorem M$'$.
\item \textbf{Hand tails.} The crude Lemma B is machine-checked to $n=2\cdot10^5$. Beyond that $E,R\le Cn^{5/2}\theta^n$ with $\theta\le0.926$ by P1e's term-by-term argument, not machine-checked. The margin is absurd: $E(2\cdot10^5)\le e^{-15388}<10^{-6682}$ (printed log-bound).
