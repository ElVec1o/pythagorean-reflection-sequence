...
# Look in this script's own directory first, then one level up.  This file used
# to live in route_b/, one level below zeta_probe/, so the parent was the only
# place it looked; it now sits in zeta_probe/ itself, alongside lamp_formula.py.
def _find(name):
    for d in (HERE, os.path.dirname(HERE)):
        p=os.path.join(d,name)
        if os.path.exists(p): return p
    raise FileNotFoundError(f"{name} not found in {HERE} or its parent")
_S=list(sys.argv)
spec=importlib.util.spec_from_file_location("lf",_find("lamp_formula.py"))
