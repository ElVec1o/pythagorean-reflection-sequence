...
def tits(N1):
    """Tits geometric representation on N1 = n+1 generators. Faithful (Tits)."""
    TG=[]
    for i in range(N1):
...
for a in tuples:
    ...
    best=None
    for g in ["0","1","2","3"]:
        v=g+u+g[::-1]; cc=u+v+u[::-1]+v[::-1]
        if tword(cc)==TI: continue
        gl=len(reduce_word(cc))
        if best is None or gl<best[1]: best=(cc,gl,g)
    c,geo,gbest = best
    cnt = True
    # rho_a(c)=1 is forced: rho_a(u) is a translation, conjugates of translations are
    # translations, and translations commute.
    print(f"{str(a):>10}  {len(u):>4}  {str(unt):>11}  {str(lin_ok):>8}   {len(c):>4}  {geo:>5}  g={gbest}  {'forced' if (lin_ok and unt) else '-'}")
